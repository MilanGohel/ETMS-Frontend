...
export const PermissionStateService = signalStore(
        ...
        permissionTree: computed<TreeNode[]>(() => {
            ...
            if (store.permissions() && store.permissions().length > 0) {
                for (const p of store.permissions()) {
                    ...
                    if (!nodes[key]) {
                        nodes[key] = { key: p.id.toString(), label: key, children: [] };
                        tree.push(nodes[key]);
                    }
            ...
            tree.forEach(node => {
                ...
                if (childSelectedCount === node.children!.length) {
                    // all children selected → mark parent selected
                    node.partialSelected = false;
                    node.data = { ...node.data, selected: true }; // optional if you need to persist
                } else if (childSelectedCount > 0) {
                    // some selected → mark parent partial
    ...
    withMethods((store,
        ...
        toastService = inject(ToastService)) => ({
                        ...
                        return api.assignPermissionToRole(roleId, permissionId).pipe(
                            tap({
                                next: (response) => {
                                    ...
                                    // Case 2: Success!
                                    toastService.messageService.clear();
            ...
            updateRolePermissions: rxMethod<{ updateRolePermissionDto: UpdateRolePermissionsDto }>(
                pipe(
                    switchMap(({ updateRolePermissionDto }) => {
                        return api.updateRolePermissions(updateRolePermissionDto).pipe(
                            tap({
                                next: (response) => {
                                    if (!response.succeeded) {
                                    ...
                                    toastService.messageService.clear();
                                    toastService.success("Success", "Permissions Saved!");
                                },
                                error: (err: HttpErrorResponse) => {
                                    toastService.error('API Error', err.error?.message || 'Could not revoke permission.');
                                }
