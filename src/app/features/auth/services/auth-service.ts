...
import { GoogleLoginDto } from '../models/google-login.model';
...
export class AuthService {
  ...
  store = inject(CurrentUserStateStore);
  isAuthenticated$ = new BehaviorSubject<boolean | null>(null);

  getUser(): Observable<ApiResponse<UserDto> | null> {
    return this.http.get<ApiResponse<UserDto>>(`${this.apiUrl}/api/auth/me`, {
  ...
  isValidTokens$(): Observable<boolean> {
    ...
    if(this.store.currentUser()())
      console.log(this.store.currentUser() + "this is current user")
    ...
    if (!accessToken || this.isTokenExpired(accessToken)) {
      ...
      return this.refreshToken().pipe(
        switchMap(() => {
          ...
          if (this.store.currentUser()()) return of(true);
